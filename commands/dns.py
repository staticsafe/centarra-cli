...
def edit_record(args, flags):
    update_dns_zones()
    reply = centarra("/dns/zone/%s/record/%s" % (substitutes.swap("dns zones", args[0]), args[1]), subdomain=args[2], content=args[3])
...
@hook.command("dns record", args_amt=2, doc=("View information regarding a certain record in a DNS zone.", "Usage:", "\tdns record <zone> <record_id>"))
def record(args, flags):
    update_dns_zones()
    reply = centarra("/dns/zone/{}/record/{}".format(substitutes.swap("dns zones", args[0]), args[1]))
    return JsonResponse(reply, "Record #{id}\n\tTTL {ttl}\n\tPrio {prio}\n\tType {type}\n\t{name} = '{content}'".format(**reply['record']))
...
def add_record(args, flags):
    if args[1] == "valid":
        reply = centarra('/dns/zone/%s/record/new' % substitutes.swap("dns zones", args[0]))
        ...
        send = {"subdomain": args[1], "type": args[2], "content": args[3], 'ttl': flags.get('t', 300),
                "prio": flags.get('p', 0)}
        reply = centarra("/dns/zone/%s/record/new" % substitutes.swap("dns zones", args[0]), **send)
...
def delete_record(args, flags):
    update_dns_zones()
    reply = centarra("/dns/zone/%s/record/%s/delete" % (substitutes.swap("dns zones", args[0]), args[1]))
...
def delete(args, flags):
    update_dns_zones()
    reply = centarra("/dns/zone/%s/delete" % substitutes.swap("dns zones", args[0]))
...
def axfr_import(args, flags):
    update_dns_zones()
    reply = centarra("/dns/zone/%s/axfr-import" % substitutes.swap("dns zones", args[0]), nameserver=args[1])
